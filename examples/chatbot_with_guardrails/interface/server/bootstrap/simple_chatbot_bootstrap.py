...
def create_chatbot_with_guardrails_container(
    ...
    chatbot_with_guardrails_container.wire(
...
def setup_chatbot_with_guardrails_routes(app: FastAPI) -> None:
    ...
    app.include_router(
        router=chatbot_router.router, prefix="/v1", tags=["ChatbotWithGuardrails"]
    )


def bootstrap_chatbot_with_guardrails_domain(
    app: FastAPI, chatbot_with_guardrails_container: ChatbotWithGuardrailsContainer
) -> None:
    """Bootstrap the chatbot-with-guardrails example domain on the server."""
    create_chatbot_with_guardrails_container(chatbot_with_guardrails_container=chatbot_with_guardrails_container)
