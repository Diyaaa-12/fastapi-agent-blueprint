...
- **Centralized error handling (IC-195-1)**: admin page handlers route unhandled errors through `AdminErrorHandler` / `@admin_error_boundary` (`src/_core/infrastructure/admin/error_handler.py`). Raw `str(exc)` is NEVER surfaced to the UI — only a 4xx `BaseCustomException.message` is shown (as a `warning`); `>= 500` domain errors and arbitrary exceptions show a generic message (`negative`). Full detail (`context`, `admin_user`, `error_type`, `error_code`; `request_id` auto-injected) goes to the structured server log only. Critical failures redirect to `/admin/error`, which is the fourth gate-exempt route (IC-155-4): it has no auth gate (a critical error may itself be a DB/auth outage and the gate hits the DB), performs no DB/session/`admin_layout` access, and echoes only a regex-validated correlation id passed via `?rid=`.
